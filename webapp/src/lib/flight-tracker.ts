...
export function handlePosition(deviceId: string, pos: AircraftPosition): void {
  ...
  if (!tr.flightId) {
    const open = [...s.flights].reverse().find(
      (f) => f.deviceId === deviceId && !f.landingTime);
    if (open) {
      tr.flightId = open.id;
  ...
  // ── 着陸 ──
  if (tr.wasHigh && agl < AIRBORNE_CONFIRM_AGL_M && speedMs < LANDING_SPEED_MS) {
    if (flight) {
      const t = clockStr();
      s.flights = s.flights.map((f) => (f.id === flight.id ? { ...f, landingTime: t } : f));
    }
    tr.phase = "ground";
    tr.flightId = null;
    tr.takeoffMs = null;
